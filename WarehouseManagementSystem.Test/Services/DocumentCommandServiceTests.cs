...
namespace WarehouseManagementSystem.Tests.Services
{
    public class DocumentCommandServiceTests
    {
        ...
        public async Task CreateDocumentAsync_ShouldThrow_WhenNoItems()
        {
            ...
            await act.Should().ThrowAsync<ArgumentException>().WithMessage("*at least one item*");
        }

        [Fact]
        public async Task CreateDocumentAsync_ShouldAddDocumentToUnitOfWork()
        {
            ...
            _unitOfWorkMock.Verify(x => x.Documents.Add(It.Is<Document>(d => d == document)), Times.Once);
            _unitOfWorkMock.Verify(x => x.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Once);

            document.Number.Should().Be("DOC-001");
            document.Items.Count.Should().Be(draftItems.Count);
        }

        [Fact]
        public async Task StartTransferAsync_ShouldCallStartTransferOnDocument()
        {
            var doc = new Document("DOC-001", DateTime.UtcNow, DocumentType.PZ, Guid.NewGuid(), Guid.NewGuid(), null, null);
            _unitOfWorkMock.Setup(x => x.Documents.FindAsync(doc.Id))
                .ReturnsAsync(doc);
        ...
        [Fact]
        public async Task ConfirmDocumentAsync_ShouldCallStockServiceBasedOnDocumentType()
        {
            var productId = Guid.NewGuid();
            var warehouseId = Guid.NewGuid();
            var zoneId = Guid.NewGuid();

            var doc = new Document("DOC-001", DateTime.UtcNow, DocumentType.PZ, Guid.NewGuid(), warehouseId, null, null);
        ...
        [Fact]
        public async Task CancelDocumentAsync_ShouldReleaseReservationsForWZAndUpdateDocument()
        {
            var doc = new Document("DOC-001", DateTime.UtcNow, DocumentType.WZ, Guid.NewGuid(), Guid.NewGuid(), null, null);
            var reservationId = Guid.NewGuid();

            var reservationMockList = new List<StockReservation>
            {
                new StockReservation(Guid.NewGuid(), 2, "Test", Guid.NewGuid(), null)
            };

            _unitOfWorkMock.Setup(x => x.Documents.FindAsync(doc.Id))
                .ReturnsAsync(doc);

            _unitOfWorkMock.Setup(x => x.Stocks.GetActiveReservationsByDocumentIdAsync(doc.Id))
                .ReturnsAsync(reservationMockList);

            await _service.CancelDocumentAsync(doc.Id);

            foreach (var res in reservationMockList)
            {
                _stockServiceMock.Verify(x => x.ReleaseReservationAsync(res.StockId, res.Id), Times.Once);
            }
