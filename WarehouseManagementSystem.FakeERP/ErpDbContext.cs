...
public sealed class ErpDbContext(DbContextOptions<ErpDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder b)
    {
        ...
        b.Entity<ErpProcessedMessage>(e =>
        {
            e.HasKey(x => x.Id);
            e.Property(x => x.Consumer).HasMaxLength(100).IsRequired();
            e.HasIndex(x => new { x.Consumer, x.MessageId }).IsUnique();
        });
    }
}
public sealed class ErpWarehouseOrder
{
    public Guid Id { get; set; }
    public string ExternalOrderId { get; set; } = null!; public Guid CorrelationId { get; set; }
    public string Status { get; set; } = "Pending"; public Guid? WmsDocumentId { get; set; }
    public DateTimeOffset CreatedAt { get; set; }
    public DateTimeOffset? ConfirmedAt { get; set; }
}
public sealed class ErpOutboxMessage
{
    public Guid Id { get; set; }
...
public sealed class ErpProcessedMessage
{
    public Guid Id { get; set; }
    public Guid MessageId { get; set; }
