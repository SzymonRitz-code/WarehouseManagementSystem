...
using (var scope = host.Services.CreateScope())
{
    ...
    if (args.Contains("--demo-order") && !await db.WarehouseOrders.AnyAsync(x => x.ExternalOrderId == "ERP-DEMO-001"))
    {
        var correlation = Guid.NewGuid();
        var order = new ErpWarehouseOrder
        {
            Id = Guid.NewGuid(),
        ...
        db.WarehouseOrders.Add(order);
        db.OutboxMessages.Add(new ErpOutboxMessage
        {
            Id = Guid.NewGuid(),
