        ...
        consumer.Received += (_, delivery) =>
        {
            ...
            catch (Exception ex)
            {
                ...
                if (policy.ShouldRetry(retries))
                {
                    ...
                    channel.BasicAck(delivery.DeliveryTag, false);
                    logger.LogWarning(ex, "FakeBilling decision retry {RetryCount}. MessageId {MessageId}, CorrelationId {CorrelationId}", policy.NextRetryCount(retries), delivery.BasicProperties.MessageId, delivery.BasicProperties.CorrelationId);
                    return;
                }

                logger.LogError(ex, "FakeBilling decision dead-letter after {RetryCount} retries. MessageId {MessageId}, CorrelationId {CorrelationId}", retries, delivery.BasicProperties.MessageId, delivery.BasicProperties.CorrelationId);
...
public sealed class BillingRabbitMqTopology(IOptions<BillingMessagingOptions> options)
{
    public void EnsureTopology(IModel channel)
    {
        ...
        channel.QueueDeclare(o.RetryQueue, true, false, false, new Dictionary<string, object> { ["x-message-ttl"] = o.RetryDelaySeconds * 1000, ["x-dead-letter-exchange"] = o.Exchange, ["x-dead-letter-routing-key"] = o.RoutingKey });
        channel.QueueBind(o.RetryQueue, o.RetryExchange, o.RoutingKey);
        channel.QueueDeclare(o.Queue, true, false, false, new Dictionary<string, object> { ["x-dead-letter-exchange"] = o.DeadLetterExchange, ["x-dead-letter-routing-key"] = o.RoutingKey });
...
public sealed class BillingConsumerRetryPolicy
{
    ...
    public BillingConsumerRetryPolicy(int maxRetryAttempts) => _maxRetryAttempts = Math.Max(0, maxRetryAttempts);
    ...
    public static int GetRetryCount(IDictionary<string, object>? headers) => headers is not null && headers.TryGetValue(RetryCountHeader, out var value) && value is not null ? Convert.ToInt32(value, System.Globalization.CultureInfo.InvariantCulture) : 0;
}
